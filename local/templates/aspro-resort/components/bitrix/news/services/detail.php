								...
								<div class="item review" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
									<div class="it">
										...
										<?if($arItem['PROPERTY_DOCUMENTS_VALUE']):?>
											<div class="row docs">
												<?if(!is_array($arItem['PROPERTY_DOCUMENTS_VALUE'])){
													$arItem['PROPERTY_DOCUMENTS_VALUE']=array($arItem['PROPERTY_DOCUMENTS_VALUE']);
												}?>
												<?foreach($arItem['PROPERTY_DOCUMENTS_VALUE'] as $docID):?>
													<?$arFile = CResort::get_file_info($docID);?>
													<div class="col-md-6 item">
														...
														<div class="text">
															...
															<?=CResort::filesize_format($arFile['FILE_SIZE']);?>
														</div>
													</div>
												<?endforeach;?>
											</div>
		...
		<?if(in_array('LINK_STAFF', $arParams['DETAIL_PROPERTY_CODE']) && $arElement['PROPERTY_LINK_STAFF_VALUE']):?>
			<div class="wraps nomargin">				
				...
				<?$APPLICATION->IncludeComponent("bitrix:news.list", "staff-linked", array(
					...
					"IBLOCK_ID" => CCache::$arIBlocks[SITE_ID]["aspro_resort_content"]["aspro_resort_staff"][0],
					"NEWS_COUNT" => "30",
					"SORT_BY1" => "SORT",
					"SORT_ORDER1" => "DESC",
					"SORT_BY2" => "",
