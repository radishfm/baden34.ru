	...
	if (null == window.bGoogleMapScriptsLoaded)
	{
		...
		else
		{
			...
			BX.loadScript(
				...
				{
					if (BX.browser.IsIE())
						setTimeout("window.google.load('maps', <?= intval($arParams['GOOGLE_VERSION'])?>, {callback: init_<?echo $arParams['MAP_ID']?>, other_params: 'sensor=false&language=<?= LANGUAGE_ID?>&key=<?=$arParams['API_KEY']?>'})", 1000);
					else
						google.load('maps', <?echo intval($arParams['GOOGLE_VERSION'])?>, {callback: init_<?echo $arParams['MAP_ID']?>, other_params: 'sensor=false&language=<?=LANGUAGE_ID?>&key=<?=$arParams['API_KEY']?>'});
				}
			);
		}
	}
