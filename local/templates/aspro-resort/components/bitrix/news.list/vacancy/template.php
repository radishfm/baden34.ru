						...
						<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
							...
							<?foreach($arSection['ITEMS'] as $i => $arItem):?>
								...
								<?ob_start();?>
									...
									<?endif;?>
									<button class="btn btn-default" data-event="jqm" data-name="resume" data-param-id="<?=CCache::$arIBlocks[SITE_ID]['aspro_resort_form']['aspro_resort_resume'][0]?>" data-autoload-POST="<?=$arItem['NAME']?>" data-autohide=""><?=GetMessage('BUTTON')?></button>
