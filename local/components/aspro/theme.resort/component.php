...
if(!CModule::IncludeModule('aspro.resort')){
	?>
	<div class='alert alert-warning'><?=GetMessage('RESORT_MODULE_NOT_INSTALLED')?></div>
	<?
	die();
}

$arFrontParametrs = CResort::GetFrontParametrsValues(SITE_ID);
$arResult = array();
foreach(CResort::$arParametrsList as $blockCode => $arBlock){
...
$APPLICATION->AddHeadString(CResort::GetBannerStyle($arResult['BANNER_WIDTH']['VALUE'], $arResult['TOP_MENU']['VALUE']), true);
