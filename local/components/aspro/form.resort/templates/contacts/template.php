				...
				<div class="form contacts<?=($arResult['isFormNote'] == 'Y' ? ' success' : '')?><?=($arResult['isFormErrors'] == 'Y' ? ' error' : '')?>">
					<?if( $arResult["isFormNote"] == "Y" ){?>
						...
						$(document).ready(function(){
							if(arResortOptions['THEME']['USE_FORMS_GOALS'] !== 'NONE'){
								var eventdata = {goal: 'goal_webform_success' + (arResortOptions['THEME']['USE_FORMS_GOALS'] === 'COMMON' ? '' : '_<?=$arParams["IBLOCK_ID"]?>'), params: <?=CUtil::PhpToJSObject($arParams, false)?>, result: <?=CUtil::PhpToJSObject($arResult, false)?>};
...
$(document).ready(function(){
	...
	$('.contacts form[name="<?=$arResult["IBLOCK_CODE"]?>"] input.datetime').inputmask(arResortOptions['THEME']['DATETIME_MASK'], { 'placeholder': arResortOptions['THEME']['DATETIME_PLACEHOLDER'] });
